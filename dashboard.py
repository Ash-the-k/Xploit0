...
def main():
    ...
    # Short summary table (without full description)
    summary_df = filtered_df[['CVE_ID', 'Package', 'CVSS', 'Predicted_Severity']].copy()
    st.dataframe(summary_df, use_container_width=True)

    # Add expandable descriptions
    st.markdown("### 🔍 Full Descriptions")
    for idx, row in filtered_df.iterrows():
        with st.expander(f"{row['CVE_ID']} - {row['Package']}"):
            st.markdown(f"""
