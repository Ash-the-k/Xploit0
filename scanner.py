...
def try_ftp_login(ip, user, password):
    try:
...
def brute_force_ftp(ip):
    print("[+] Starting multi-threaded FTP brute force...")
    user_file = 'user.txt'
    pass_file = 'passwords.txt'
    
    # Open files with UTF-8 encoding
    try:
        with open(user_file, encoding="utf-8") as u:
            users = u.readlines()
        with open(pass_file, encoding="utf-8") as p:
            passwords = p.readlines()
        ...
        for user in users:
            for password in passwords:
                ...
                # Limit active threads to prevent overload
        ...
        print(f"[!] Error: Could not find {user_file} or {pass_file}")
    except Exception as e:
        print(f"[!] Error during FTP brute force: {str(e)}")
